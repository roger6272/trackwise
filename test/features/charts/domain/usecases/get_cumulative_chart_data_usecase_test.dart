...
void main() {
  late GetCumulativeChartDataUseCase useCase;
  ...
  setUp(() {
    ...
    useCase = GetCumulativeChartDataUseCase(mockRepository, mockItemRepository);
  });

  group('GetCumulativeChartDataUseCase', () {
    test('should calculate cumulative totals correctly', () async {
      // Arrange
      ...
      result.fold(
        ...
        (chartData) {
          ...
          // Jan 15: 3 + 2 = 5 (cumulative: 5)
          expect(chartData.dataPoints[0].value, 5);
          // Jan 16: 10 (cumulative: 5 + 10 = 15)
          expect(chartData.dataPoints[1].value, 15);
          // Jan 17: 3 (cumulative: 15 + 3 = 18)
          expect(chartData.dataPoints[2].value, 18);
        },
      );
    });
    ...
    test('should handle single event correctly', () async {
      // Arrange
      final singleEvent = [testEventsForAggregation.first];
      when(() => mockRepository.getEventsByDateRange(any(), any()))
      ...
      result.fold(
        (failure) => fail('Should not fail'),
        (chartData) {
          expect(chartData.dataPoints.length, 1);
          expect(chartData.dataPoints[0].value, 3); // Single event increment
        },
      );
    });
  });
